...
$db->query("DELETE FROM mesajlar WHERE uye_no='$get->uye' AND urun_no='$get->urun'");
...
if(!$hata){
$db->query("INSERT INTO mesajlar (yazan,uye_no,urun_no,mesaj,zaman) VALUES ('$session->UYE_no','$post->uye_no','$post->urun','$post->mesaj','$zaman')");
...
if($get->uye){
$sqlu=$db->query("SELECT * FROM uyeler WHERE no='$get->uye'");
...
$sql=$db->query("SELECT * FROM mesajlar WHERE  uye_no='$get->uye' ORDER BY no");
while($s=$sql->fetch_object()
{
$db->query("UPDATE mesajlar SET yonetim_ziyaret=1 WHERE no='$s->no'");
