...
class Storage extends \Core\Api{
    ...
    public static  function url($dosya){
        return "http://".$_SERVER['HTTP_HOST']."/".STORAGE_PATH.$dosya;
    }

    public static  function fullPath($dosya){
        return STORAGE_FULLPATH."/".$dosya;
    }
    ...
    public static  function upload($tmp_adres){
        ...
        if(!file_exists(STORAGE_FULLPATH)){
            mkdir(STORAGE_FULLPATH);
            chmod(STORAGE_FULLPATH,0777);
        }
        ...
        if($d=move_uploaded_file($tmp_adres, STORAGE_FULLPATH."/$ad")){
