...
$db->query("UPDATE uyeler SET kullanici='$post->kullanici' WHERE no=$get->no")or die($db->error());
$db->query("UPDATE uyeler SET yetki='$post->yetki' WHERE no=$get->no")or die($db->error());
 $db->query("UPDATE uyeler SET ad='$post->ad' WHERE no=$get->no")or die($db->error());
$db->query("UPDATE uyeler SET soyad='$post->soyad' WHERE no=$get->no")or die($db->error());
 $db->query("UPDATE uyeler SET telefon='$post->telefon' WHERE no=$get->no")or die($db->error());
$db->query("UPDATE uyeler SET eposta='$post->eposta' WHERE no=$get->no")or die($db->error());
$db->query("UPDATE uyeler SET adres='$post->adres' WHERE no=$get->no")or die($db->error());
$db->query("UPDATE uyeler SET il='$post->il' WHERE no=$get->no")or die($db->error());
$db->query("UPDATE uyeler SET ilce='$post->ilce' WHERE no=$get->no")or die($db->error());

if($post->parola)
if($post->parola==$post->parola_tekrar)
{
$db->query("UPDATE uyeler SET parola='".md5($post->parola)."' WHERE no=$get->no")or die($db->error());
...
$sql=$db->query("SELECT * FROM uyeler WHERE no=$get->no");
$s=$sql->fetch_object(;
